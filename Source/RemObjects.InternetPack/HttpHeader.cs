...
namespace RemObjects.InternetPack.Http
{
    ...
    public class HttpHeaders : IEnumerable
    {
        ...
        public void ParseFirstLine()
        {
            ...
            {
                // HTTP Response
                this.fResponseCode = lRequestHeaderValues[1];

                StringBuilder lResponseText = new StringBuilder(24);
                for (Int32 i = 2; i < lRequestHeaderValues.Length; i++)
                {
                    if (i > 2)
                        lResponseText.Append(' ');
                    lResponseText.Append(lRequestHeaderValues[i]);
                }
                this.fResponseText = lResponseText.ToString();
            }
            else
            {
                // HTTP Request
        ...
        public void SetResponseHeader(String version, Int32 resultCode, String resultText)
        {
            fFirstHeader = String.Format("HTTP/{0} {1} {2}", version, resultCode, resultText);
        }
    ...
    [Serializable]
    public class HttpHeaderException : System.ApplicationException
    {
