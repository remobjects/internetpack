...
namespace RemObjects.InternetPack.Ldap
{
    ...
    public class LdapClient : Client
    {
        ...
        private DigestType fBindDigest;

        public Int32 LdapVersion
        {
            get
            {
                return fLdapVersion;
        ...
        private String fBindDN;

        public String BindPassword
        {
            get
            {
                return fBindPassword;
            }
            set
            {
                fBindPassword = value;
        ...
        private class Response
        {
            ...
            public Int32 TypeCode; //type code tag
            public Int32 Code;
            public String DN;
            public String Result;
            public BerSequence Referers; // referers
            public BerValue[] RestData; // rest data; only valid when it's not referers
        }
        ...
        public void Bind()
        {
            this.Bind(fBindDN, fBindPassword, fBindDigest);
        }
                                    ...
                                    if (((BerSequence)attribute).Items.Count > 1 && ((BerSequence)attribute).Items[1].Type == BerType.Sequence)
                                    {
                                        foreach (BerValue value in ((BerSequence)((BerSequence)attribute).Items[1]).Items)
                                        {
                                            switch (value.Type)
                                            {
                                                ...
                                                case BerType.String:
                                                    att.Binary = false;
                                                    att.Add(((BerString)value).Value.ToString());
                                                    break;
