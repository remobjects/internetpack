...
type
  ...
  public
    ...
    property Value: nullable String read if fValues.Count = 1 then fValues[""]; virtual;
  ...
  public
    ...
    property HasValues: Boolean read fValues.Count > 0;

    method GetCookieHeaderString(aName: String): String;
    begin
      var lString := new StringBuilder;

      lString.Append(aName);
      lString.Append("=");

      for each k in Values.Keys index i do begin
        if i > 0 then
          lString.Append("&");
        if (k = "") and (Values.Count = 1)  then begin
          lString.Append(Value) // review this
        end
        else begin
          lString.Append(k);
          lString.Append("=");
          lString.Append(Values[k]) // encode?
        end;
      end;
